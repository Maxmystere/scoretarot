...
public class DonneDialog extends AlertDialog {
	...
	protected DonneDialog(Context context, Partie partie) {
		...
		alertDialogView = factory.inflate(R.layout.activity_new_donne, null);
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setButton(BUTTON_NEGATIVE, ctx.getString(R.string.cancel),
				new DialogInterface.OnClickListener() {
		...
		if (partie.getNbJoueurs() > 4)
			alertDialogView.findViewById(R.id.nd_lappele).setVisibility(
					View.VISIBLE);
		mort = (Spinner) alertDialogView.findViewById(R.id.nd_mort);
		mort.setAdapter(adapter);
		if (partie.getNbJoueurs() == 6)
			alertDialogView.findViewById(R.id.nd_lmort).setVisibility(
					View.VISIBLE);
		...
		((View) mAcceptButton).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (points.getText().toString().isEmpty())
					points.setText("0");
				if (bouts.getText().toString().isEmpty())
					bouts.setText("0");
				...
				if (partie.getNbJoueurs() > 5) {
					...
					if (m == preneur.getSelectedItemPosition()
							|| m == appele.getSelectedItemPosition()) {
						Toast.makeText(ctx,
								ctx.getString(R.string.mort_preneur),
